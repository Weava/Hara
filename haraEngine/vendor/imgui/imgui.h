...
namespace ImGui {
    ...
    // - Each context create its own ImFontAtlas by default. You may instance
    // one yourself and pass it to CreateContext() to share a font atlas between
    // contexts.
    ...
    //   Memory Allocators" section of imgui.cpp for details.
    ...
    // Main
    IMGUI_API ImGuiIO&
    GetIO();  // access the ImGuiIO structure (mouse/keyboard/gamepad inputs,
    ...
    IMGUI_API void
    NewFrame();  // start a new Dear ImGui frame, you can submit any command
                 // from this point until Render()/EndFrame().
    IMGUI_API void
    EndFrame();  // ends the Dear ImGui frame. automatically called by Render().
                 // If you don't need to render data (skipping rendering) you
                 // may call EndFrame() without Render()... but you'll have
                 // wasted CPU already! If you don't need to render, better to
                 // not create any windows and not call NewFrame() at all!
    ...
    GetVersion();  // get the compiled version string e.g. "1.80 WIP"
                   // (essentially the value for IMGUI_VERSION from the compiled
                   // version of imgui.cpp)
    ...
    //   call, regardless of its return value! [Important: due to legacy reason,
    //   Begin/End and BeginChild/EndChild are inconsistent with all other
    //   functions
    //    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX
    //    call should only be called if the corresponding BeginXXX function
    //    returned true. Begin and BeginChild are the only odd ones out. Will be
    ...
    //      content boundaries.
    // - Specifying ImGuiChildFlags_AutoResizeX or ImGuiChildFlags_AutoResizeY
    // makes the sizing automatic based on child contents.
    //   Combining both ImGuiChildFlags_AutoResizeX _and_
    //   ImGuiChildFlags_AutoResizeY defeats purpose of a scrolling region and
    //   is NOT recommended.
    ...
    //   legacy reason, Begin/End and BeginChild/EndChild are inconsistent with
    //   all other functions
    //    such as BeginMenu/EndMenu, BeginPopup/EndPopup, etc. where the EndXXX
    //    call should only be called if the corresponding BeginXXX function
    //    returned true. Begin and BeginChild are the only odd ones out. Will be
    ...
    IMGUI_API bool IsWindowHovered(
        ImGuiHoveredFlags flags =
            0);  // is current window hovered and hoverable (e.g. not blocked by
                 // a popup/modal)? See ImGuiHoveredFlags_ for options.
    ...
    GetWindowPos();  // get current window position in screen space (IT IS
                     // UNLIKELY YOU EVER NEED TO USE THIS. Consider always
                     // using GetCursorScreenPos() and GetContentRegionAvail()
                     // instead)
    IMGUI_API ImVec2
    GetWindowSize();  // get current window size (IT IS UNLIKELY YOU EVER NEED
                      // TO USE THIS. Consider always using GetCursorScreenPos()
                      // and GetContentRegionAvail() instead)
    IMGUI_API float
    GetWindowWidth();  // get current window width (IT IS UNLIKELY YOU EVER NEED
                       // TO USE THIS). Shortcut for GetWindowSize().x.
    IMGUI_API float
    GetWindowHeight();  // get current window height (IT IS UNLIKELY YOU EVER
                        // NEED TO USE THIS). Shortcut for GetWindowSize().y.
    ...
    IMGUI_API void SetNextWindowContentSize(
        const ImVec2&
            size);  // set next window content size (~ scrollable client area,
                    // which enforce the range of scrollbars). Not including
                    // window decorations (title bar, menu bar, etc.) nor
                    // WindowPadding. set an axis to 0.0f to leave it automatic.
                    // call before Begin()
    ...
    IMGUI_API void SetWindowSize(
        ...
        ImGuiCond cond =
            0);  // (not recommended) set current window size - call within
                 ...
                 // prefer using SetNextWindowSize(), as this may incur tearing
    ...
    IMGUI_API void SetWindowFontScale(
        float scale);  // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale
                       // if you want to scale all windows. This is an old API!
                       // For correct scaling, prefer to reload font + rebuild
    ...
    // the first call to Begin().
    // - You may instead use SetNextWindowScroll() prior to calling Begin() to
    // avoid this delay, as an alternative to using SetScrollX()/SetScrollY().
    IMGUI_API float
    ...
    IMGUI_API void PushTextWrapPos(
        float wrap_local_pos_x =
            0.0f);  // push word-wrapping position for Text*() commands. < 0.0f:
                    // no wrapping; 0.0f: wrap to end of window (or column); >
                    // 0.0f: wrap at 'wrap_pos_x' position in window local space
    IMGUI_API void PopTextWrapPos();

    // Style read access
    // - Use the ShowStyleEditor() function to interactively see/edit the
    // colors.
    IMGUI_API ImFont* GetFont();  // get current font
    IMGUI_API float
    GetFontSize();  // get current font size (= height in pixels) of current
    ...
    GetColorU32(ImGuiCol idx,
                float alpha_mul =
                    1.0f);  // retrieve given style color with style alpha
    ...
    // - You can call SameLine() between widgets to undo the last carriage
    // return and output at the right of the preceding widget.
    ...
    //    - Absolute coordinate:        GetCursorScreenPos(),
    //    SetCursorScreenPos(), all ImDrawList:: functions. -> this is the
    ...
    // is almost only ever useful to convert from window-local to absolute
    // coordinates. Try not to use it.
    IMGUI_API ImVec2
    GetCursorScreenPos();  // cursor position, absolute coordinates. THIS IS
                           // YOUR BEST FRIEND (prefer using this rather than
    ...
    IMGUI_API void SetCursorScreenPos(
        const ImVec2& pos);  // cursor position, absolute coordinates. THIS IS
                             // YOUR BEST FRIEND.
    IMGUI_API ImVec2
    GetContentRegionAvail();  // available space from current position. THIS IS
                              // YOUR BEST FRIEND.
    IMGUI_API ImVec2
    ...
    GetCursorStartPos();  // [window-local] initial cursor position, in
                          // window-local coordinates. Call GetCursorScreenPos()
                          // after Begin() to get the absolute coordinates
                          // version.
    ...
    IMGUI_API void
    EndGroup();  // unlock horizontal starting position + capture the whole
    ...
    AlignTextToFramePadding();  // vertically align upcoming text baseline to
                                // FramePadding.y so that it will align properly
                                // to regularly framed items (call if you have
    ...
    GetFrameHeightWithSpacing();  // ~ FontSize + style.FramePadding.y * 2 +
                                  // style.ItemSpacing.y (distance in pixels
                                  // between 2 consecutive lines of framed
                                  // widgets)

    // ID stack/scopes
    // Read the FAQ (docs/FAQ.md or http://dearimgui.com/faq) for more details
    // about how ID are handled in dear imgui.
    // - Those questions are answered and impacted by understanding of the ID
    // stack system:
    //   - "Q: Why is my widget not reacting when I click on it?"
    //   - "Q: How can I have widgets with an empty label?"
    //   - "Q: How can I have multiple widgets with the same label?"
    // - Short version: ID are hashes of the entire ID stack. If you are
    // creating widgets in a loop you most likely
    ...
    IMGUI_API void TextUnformatted(
        ...
        const char* text_end =
            NULL);  // raw text without formatting. Roughly equivalent to
                    ...
                    // string if 'text_end' is specified, B) it's faster, no
                    // memory copy is done, no buffer size limits, recommended
    ...
    IMGUI_API void LabelText(const char* label, const char* fmt, ...)
        IM_FMTARGS(2);  // display text+label aligned the same way as
                        // value+label widgets
    ...
    // IsItemActive, IsItemHovered, etc.) to query widget state.
    IMGUI_API bool Button(const char* label,
                          const ImVec2& size = ImVec2(0, 0));  // button
    IMGUI_API bool SmallButton(
        const char* label);  // button with (FramePadding.y == 0) to easily
                             // embed within text
    ...
    IMGUI_API bool CheckboxFlags(const char* label, int* flags,
                                 int flags_value);
    ...
    IMGUI_API void
    Bullet();  // draw a small circle + keep the cursor on the same line.
    ...
    // link above.
    // - Image() pads adds style.ImageBorderSize on each side, ImageButton()
    // adds style.FramePadding on each side.
    ...
    IMGUI_API void ImageWithBg(ImTextureID user_texture_id,
    ...
    // kept available for convenience purpose. This is analogous to how ListBox
    // are created.
    ...
    IMGUI_API bool Combo(
        ...
        int popup_max_height_in_items =
            -1);  // Separate items with \0 within a string, end item-list with
    ...
    // input values aren't clamped by default and can go off-bounds. Use
    // ImGuiSliderFlags_AlwaysClamp to always clamp.
    // - For all the Float2/Float3/Float4/Int2/Int3/Int4 versions of every
    // function, note that a 'float v[X]' function argument is the same as
    // 'float* v',
    //   the array syntax is just a way to document the number of elements that
    //   are expected to be accessible. You can pass address of your first
    ...
    // - Format string may also be set to NULL or use the default format ("%f"
    // or "%d").
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to
    // move by 5 pixels to increase value by 1). For keyboard/gamepad
    ...
    // manual input can override those limits if ImGuiSliderFlags_AlwaysClamp is
    // not used.
    ...
    // - We use the same sets of flags for DragXXX() and SliderXXX() functions
    // as the features are the same and it makes it easier to swap them.
    // - Legacy: Pre-1.78 there are DragXXX() function signatures that take a
    // final `float power=1.0f' argument instead of the `ImGuiSliderFlags
    // flags=0' argument.
    ...
    // color square that can be left-clicked to open a picker, and right-clicked
    // to open an option menu.)
    ...
    IMGUI_API bool ColorButton(
        ...
        const ImVec2& size =
            ImVec2(0, 0));  // display a color square/button, hover for details,
    ...
    IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
        IM_FMTARGS(2);  // helper variation to easily decorelate the id from the
                        // displayed string. Read the FAQ about why and how to
                        // use ID. to align arbitrary text at the same level as
    ...
    IMGUI_API float
    GetTreeNodeToLabelSpacing();  // horizontal distance preceding label when
                                  // using TreeNode*() or Bullet() ==
                                  // (g.FontSize + style.FramePadding.x*2) for a
    ...
    // gap between them. This is so a series of selected Selectable appear
    // contiguous.
    ...
    // (CTRL+Mouse/Keyboard, SHIFT+Mouse/Keyboard, etc.) in a way that also
    // allow a clipper to be used.
    // - ImGuiSelectionUserData is often used to store your item index within
    // the current view (but may store something else).
    ...
    //   which is suited to advanced trees setups already implementing filters
    //   and clipper. We will work simplifying the current demo.
    // - 'selection_size' and 'items_count' parameters are optional and used by
    // a few features. If they are costly for you to compute, you may avoid
    // them.
    ...
    IMGUI_API bool
    IsItemToggledSelection();  // Was the last item selection state toggled?
                               // Useful if you need the per-item information
                               // _before_ reaching EndMultiSelect(). We only
                               // returns toggle _event_ in order to handle
                               // clipping correctly.

    // Widgets: List Boxes
    // - This is essentially a thin wrapper to using BeginChild/EndChild with
    // the ImGuiChildFlags_FrameStyle flag for stylistic changes + displaying a
    // label.
    ...
    // -FLT_MIN: right-align   /  size.x = 0.0f (default): use current ItemWidth
    ...
    // Widgets: Menus
    // - Use BeginMenuBar() on a window ImGuiWindowFlags_MenuBar to append to
    // its menu bar.
    // - Use BeginMainMenuBar() to create a menu bar at the top of the screen
    // and append to it.
    // - Use BeginMenu() to create a menu. You can call BeginMenu() multiple
    // time with the same identifier to append more items to it.
    // - Not that MenuItem() keyboardshortcuts are displayed as a convenience
    // but _not processed_ by Dear ImGui at the moment.
    IMGUI_API bool
    ...
    IMGUI_API void SetItemTooltip(const char* fmt, ...)
        IM_FMTARGS(1);  // set a text-only tooltip if preceding item was
    ...
    //  by pressing ESCAPE.
    //  - Their visibility state (~bool) is held internally instead of being
    ...
    //  ImGuiHoveredFlags_AllowWhenBlockedByPopup when calling IsItemHovered()
    //  or IsWindowHovered().
    ...
    //  stack.
    //    This is sometimes leading to confusing mistakes. May rework this in
    //    the future.
    //  - BeginPopup(): query popup state, if open start appending into the
    //  window. Call EndPopup() afterwards if returned true. ImGuiWindowFlags
    //  are forwarded to the window.
    //  - BeginPopupModal(): block every interaction behind the window, cannot
    //  be closed by user, add a dimming background, has a title bar.
    ...
    //  - OpenPopup(): set popup state to open. ImGuiPopupFlags are available
    //  for opening options.
    //  - If not modal: they can be closed by clicking anywhere outside them, or
    //  by pressing ESCAPE.
    //  - CloseCurrentPopup(): use inside the BeginPopup()/EndPopup() scope to
    //  close manually.
    //  - CloseCurrentPopup() is called by default by Selectable()/MenuItem()
    //  when activated (FIXME: need some options).
    //  - Use ImGuiPopupFlags_NoOpenOverExistingPopup to avoid opening a popup
    //  if there's already one at the same level. This is equivalent to e.g.
    //  testing for !IsAnyPopupOpen() prior to OpenPopup().
    //  - Use IsWindowAppearing() after BeginPopup() to tell if a window just
    //  opened.
    //  - IMPORTANT: Notice that for OpenPopupOnItemClick() we exceptionally
    //  default flags to 1 (== ImGuiPopupFlags_MouseButtonRight) for backward
    ...
    //  like OpenPopup() and unlike BeginPopup(). For full consistency, we may
    //  add ImGuiWindowFlags to the BeginPopupContextXXX functions in the
    //  future.
    //  - IMPORTANT: Notice that we exceptionally default their flags to 1 (==
    //  ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older
    //  API taking 'int mouse_button = 1' parameter, so if you add other flags
    //  remember to re-add the ImGuiPopupFlags_MouseButtonRight.
    IMGUI_API bool BeginPopupContextItem(
        const char* str_id = NULL,
        ImGuiPopupFlags popup_flags =
            1);  // open+begin popup when clicked on last item. Use str_id==NULL
                 // to associate the popup to previous item. If you want to use
                 // that on a non-interactive item such as Text() you need to
    ...
    IMGUI_API bool BeginPopupContextVoid(
        const char* str_id = NULL,
        ImGuiPopupFlags popup_flags = 1);  // open+begin popup when clicked in
    ...
    //  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any
    //  popup is open at the current BeginPopup() level of the popup stack.
    ...
    // of available flags. The typical call flow is:
    // - 1. Call BeginTable(), early out if returning false.
    // - 2. Optionally call TableSetupColumn() to submit column
    // name/flags/defaults.
    ...
    // - 5. Populate contents:
    //    - In most situations you can use TableNextRow() +
    //    TableSetColumnIndex(N) to start appending into a column.
    //    - If you are using tables as a sort of grid, where every column is
    //    holding the same type of contents,
    //      you may prefer using TableNextColumn() instead of TableNextRow() +
    //      TableSetColumnIndex(). TableNextColumn() will automatically
    //      wrap-around into the next row if needed.
    ...
    //        Not OK! Missing TableSetColumnIndex() or TableNextColumn()! Text
    //        will not appear!
    // - 5. Call EndTable()
    ...
    IMGUI_API bool
    TableNextColumn();  // append into the next column (or first column of next
    ...
    //   context menu. The context menu can also be made available in columns
    //   body using ImGuiTableFlags_ContextMenuInBody.
    // - You may manually submit headers using TableNextRow() + TableHeader()
    // calls, but this is only useful in
    ...
    TableHeadersRow();  // submit a row with headers cells based on data
                        // provided to TableSetupColumn() + submit context menu
    IMGUI_API void TableAngledHeadersRow();  // submit a row with angled headers
                                             // for every column with the
    ...
    //   When 'sort_specs->SpecsDirty == true' you should sort your data. It
    //   will be true when sorting specs have changed since last call, or the
    //   first time. Make sure to set 'SpecsDirty = false' after sorting, else
    ...
    TableGetSortSpecs();  // get latest sort specs for the table (NULL if not
                          // sorting).  Lifetime: don't hold on this pointer
                          // over multiple frames or past any subsequent call to
                          // BeginTable().
    ...
    IMGUI_API void TableSetColumnEnabled(
        int column_n,
        bool v);  // change user accessible enabled/disabled state of a column.
                  // Set to false to hide the column. User can use the context
                  // menu to change this themselves (right-click in headers, or
                  // right-click in columns body with
                  // ImGuiTableFlags_ContextMenuInBody)
    IMGUI_API int
    TableGetHoveredColumn();  // return hovered column. return -1 when table is
                              // not hovered. return columns_count if the unused
    ...
    IMGUI_API void SetColumnWidth(
        int column_index, float width);  // set column width (in pixels). pass
                                         // -1 to use current column
    IMGUI_API float GetColumnOffset(
        int column_index =
            -1);  // get position of column line (in pixels, from the left side
                  // of the contents region). pass -1 to use current column,
                  // otherwise 0..GetColumnsCount() inclusive. column 0 is
                  // typically 0.0f
    ...
    // Drag and Drop
    // - On source items, call BeginDragDropSource(), if it returns true also
    // call SetDragDropPayload() + EndDragDropSource().
    // - On target candidates, call BeginDragDropTarget(), if it returns true
    // also call AcceptDragDropPayload() + EndDragDropTarget().
    // - If you stop calling BeginDragDropSource() the payload is preserved
    // however it won't have a preview tooltip (we currently display a fallback
    // "..." tooltip, see #1725)
    ...
    GetDragDropPayload();  // peek directly into the current payload from
                           ...
                           // ImGuiPayload::IsDataType() to test for the payload
                           // type.
    ...
    // provided to facilitate use of boolean expressions (as a
    // micro-optimization: if you have tens of thousands of
    ...
    SetNextItemAllowOverlap();  // allow next item to be overlapped by a
                                // subsequent item. Useful with invisible
                                // buttons, selectable, treenode covering an
                                // area where subsequent items may need to be
                                // added. Note that both Selectable() and
    ...
    IMGUI_API bool
    IsItemActive();  // is the last item active? (e.g. button being held, text
    ...
    IMGUI_API bool
    IsItemVisible();  // is the last item visible? (items may be out of sight
    ...
    IsItemDeactivatedAfterEdit();  // was the last item just made inactive and
                                   ...
                                   // require continuous editing. Note that you
    ...
    GetMainViewport();  // return primary/default viewport. This can never be
                        // NULL.
    ...
    GetForegroundDrawList();  // this draw list will be the last rendered one.
                              // Useful to quickly draw shapes/text over dear
                              // imgui contents.
    ...
    // - the ImGuiKey enum contains all possible keyboard, mouse and gamepad
    // inputs (e.g. ImGuiKey_A, ImGuiKey_MouseLeft, ImGuiKey_GamepadDpadUp...).
    // - (legacy: before v1.87, we used ImGuiKey to carry native/user indices as
    // defined by each backends. This was obsoleted in 1.87 (2022-02) and
    // completely removed in 1.91.5 (2024-11). See
    // https://github.com/ocornut/imgui/issues/4921)
    // - (legacy: any use of ImGuiKey will assert when key < 512 to detect
    // passing legacy native/user indices)
    IMGUI_API bool IsKeyDown(ImGuiKey key);  // is key being held.
    IMGUI_API bool IsKeyPressed(
        ImGuiKey key,
        bool repeat =
            true);  // was key pressed (went from !Down to Down)? if
    ...
    IMGUI_API const char* GetKeyName(
        ImGuiKey key);  // [DEBUG] returns English name of the key. Those names
                        // are provided for debugging purpose and are not meant
                        // to be saved persistently nor compared.
    IMGUI_API void SetNextFrameWantCaptureKeyboard(
        bool want_capture_keyboard);  // Override io.WantCaptureKeyboard flag
                                      // next frame (said flag is left for your
                                      // application to handle, typically when
                                      // true it instructs your app to ignore
                                      // inputs). e.g. force capture keyboard
                                      // when your widget is being hovered. This
    ...
    //   only ImGuiMod_XXX values are legal to combine with an ImGuiKey. You
    //   CANNOT combine two ImGuiKey values.
    // - The general idea is that several callers may register interest in a
    // shortcut, and only one owner gets it.
    //      Parent   -> call Shortcut(Ctrl+S)    // When Parent is focused,
    //      Parent gets the shortcut.
    //        Child1 -> call Shortcut(Ctrl+S)    // When Child1 is focused,
    //        Child1 gets the shortcut (Child1 overrides Parent shortcuts)
    //        Child2 -> no call                  // When Child2 is focused,
    //        Parent gets the shortcut.
    //   The whole system is order independent, so if Child1 makes its calls
    //   before Parent, results will be identical. This is an important property
    //   as it facilitate working with foreign code or larger codebase.
    ...
    //   be routed it calls IsKeyChordPressed() -> function has (desirable)
    ...
    // - One common use case would be to allow your items to disable standard
    // inputs behaviors such
    ...
    // distance away from the initial clicking position (see 'lock_threshold'
    // and 'io.MouseDraggingThreshold')
    IMGUI_API bool IsMouseDown(
        ImGuiMouseButton button);  // is mouse button held?
    ...
    IMGUI_API bool IsMouseDoubleClicked(
        ImGuiMouseButton
            button);  // did mouse button double-clicked? Same as
                      // GetMouseClickedCount() == 2. (note that a double-click
                      // will also report IsMouseClicked() == true)
    IMGUI_API bool IsMouseReleasedWithDelay(
        ImGuiMouseButton button,
        float
            delay);  // delayed mouse release (use very sparingly!). Generally
                     // used with 'delay >= io.MouseDoubleClickTime' + combined
                     // with a 'io.MouseClickedLastCount==1' test. This is a
                     // very rarely used UI idiom, but some apps use this: e.g.
                     // MS Explorer single click on an icon to rename.
    ...
    IsAnyMouseDown();  // [WILL OBSOLETE] is any mouse button held? This was
                       // designed for backends, but prefer having backend
                       // maintain a mask of held mouse buttons, because
    ...
    IMGUI_API ImVec2
    GetMousePosOnOpeningCurrentPopup();  // retrieve mouse position at the time
    ...
    IMGUI_API ImVec2 GetMouseDragDelta(
        ...
        float lock_threshold =
            -1.0f);  // return the delta from the initial clicking position
                     // while the mouse button is pressed or was just released.
                     // This is locked and return 0.0f until the mouse moves
                     // past a distance threshold at least once (uses
    ...
    GetMouseCursor();  // get desired mouse cursor shape. Important: reset in
                       // ImGui::NewFrame(), this is updated during the frame.
                       // valid before Render(). If you use software rendering
                       // by setting io.MouseDrawCursor ImGui will render those
                       // for you
    ...
    IMGUI_API void SetNextFrameWantCaptureMouse(
        bool
            want_capture_mouse);  // Override io.WantCaptureMouse flag next
                                  // frame (said flag is left for your
                                  // application to handle, typical when true it
                                  // instucts your app to ignore inputs). This
                                  // is equivalent to setting
                                  // "io.WantCaptureMouse = want_capture_mouse;"
                                  // after the next NewFrame() call.

    // Clipboard Utilities
    // - Also see the LogToClipboard() function to capture GUI into clipboard,
    ...
    // - Important: default value "imgui.ini" is relative to current working
    // dir! Most apps will want to lock this to an absolute path (e.g. same path
    // as executables).
    ...
    IMGUI_API void SaveIniSettingsToDisk(
        const char*
            ini_filename);  // this is automatically called (if io.IniFilename
                            ...
                            // modification that should be reflected in the .ini
                            // file (and also by DestroyContext).
    IMGUI_API const char* SaveIniSettingsToMemory(
        size_t* out_ini_size =
            NULL);  // return a zero-terminated string with the .ini data which
                    // you can save by your own mean. call when
                    // io.WantSaveIniSettings is set, then save data by your own
                    // mean and clear io.WantSaveIniSettings.

    // Debug Utilities
    // - Your main debugging friend is the ShowMetricsWindow() function, which
    // is also accessible from Demo->Tools->Metrics Debugger
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
        1 << 17,  // (WIP) Nav: left direction may move to this TreeNode() from
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_AlphaOpaque =
        1 << 11,  //              // ColorEdit, ColorPicker, ColorButton:
    ...
    ImGuiColorEditFlags_AlphaPreviewHalf =
        1
        << 13,  //              // ColorEdit, ColorPicker, ColorButton: display
...
enum ImGuiSliderFlags_ {
    ...
    ImGuiSliderFlags_InvalidMask_ =
        0x7000000F,  // [Internal] We treat using those bits as being
                     ...
                     // API that has got miscast to this enum, and will trigger
                     // an assert if needed.
};
...
struct ImFontConfig {
    ...
    // ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED IN 1.91.9:
    // use GlyphExtraAdvanceX)
    ...
    float
        RasterizerDensity;  // 1.0f     // DPI scale for rasterization, not
                            // altering other font metrics: make it easy to swap
                            // between e.g. a 100% and a 400% fonts for a
...
struct ImFontAtlasCustomRect {
    ...
    unsigned int GlyphID : 31;  // Input    // For custom font glyphs only (ID <
                                // 0x110000)
...
struct ImFont {
    ...
    ImFontConfig*
        Sources;  // 4-8   // in  // Pointer within ContainerAtlas->Sources[],
                  // to SourcesCount instances
    short SourcesCount;  // 2     // in  // Number of ImFontConfig involved in
...
namespace ImGui {
    ...
    GetContentRegionMax();  // Content boundaries max (e.g. window boundaries
                            // including scrolling, or current column
                            // boundaries). You should never need this. Always
                            // use GetCursorScreenPos() and
                            // GetContentRegionAvail()!
    IMGUI_API ImVec2
    GetWindowContentRegionMin();  // Content boundaries min for the window
                                  // (roughly (0,0)-Scroll), in window-local
                                  // coordinates. You should never need this.
                                  // Always use GetCursorScreenPos() and
                                  // GetContentRegionAvail()!
    IMGUI_API ImVec2
    GetWindowContentRegionMax();  // Content boundaries max for the window
                                  // (roughly (0,0)+Size-Scroll), in
                                  // window-local coordinates. You should never
                                  // need this. Always use GetCursorScreenPos()
    ...
    // Some of the older obsolete names along with their replacement (commented
    // out so they are not reported in IDE)
    //-- OBSOLETED in 1.89 (from August 2022)
    // IMGUI_API bool      ImageButton(ImTextureID user_texture_id, const
    // ImVec2& size, const ImVec2& uv0 = ImVec2(0, 0), const ImVec2& uv1 =
    // ImVec2(1, 1), int frame_padding = -1, const ImVec4& bg_col = ImVec4(0, 0,
    // 0, 0), const ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // --> Use new
    // ImageButton() signature (explicit item id, regular FramePadding). Refer
    // to code in 1.91 if you want to grab a copy of this version.
    //-- OBSOLETED in 1.88 (from May 2022)
    // static inline void  CaptureKeyboardFromApp(bool want_capture_keyboard =
    // true)                   {
    ...
    // SetNextFrameWantCaptureMouse(want_capture_mouse); }       // Renamed as
    // name was misleading + removed default value.
    //-- OBSOLETED in 1.87 (from February 2022, more formally obsoleted April
    //2024) IMGUI_API ImGuiKey  GetKeyIndex(ImGuiKey key); { IM_ASSERT(key >=
    // ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END); const
    // ImGuiKeyData* key_data = GetKeyData(key); return (ImGuiKey)(key_data -
    // g.IO.KeysData); } // Map ImGuiKey_* values into legacy native key index.
    // == io.KeyMap[key]. When using a 1.87+ backend using io.AddKeyEvent(),
    // calling GetKeyIndex() with ANY ImGuiKey_XXXX values will return the same
    // value! static inline ImGuiKey GetKeyIndex(ImGuiKey key) { IM_ASSERT(key
    ...
    // int* out_items_display_start, int* out_items_display_end); // Code
    // removed, see 1.90 for last version of the code. Calculate range of
    // visible items for large list of evenly sized items. Prefer using
    // ImGuiListClipper.
    ...
    // bool  ListBoxHeader(const char* label, int items_count, int
    // height_in_items = -1) { float height = GetTextLineHeightWithSpacing() *
    // ((height_in_items < 0 ? ImMin(items_count, 7) : height_in_items) + 0.25f)
    // + GetStyle().FramePadding.y * 2.0f; return BeginListBox(label,
    // ImVec2(0.0f, height)); } // Helper to calculate size from items_count and
    ...
    //-- OBSOLETED in 1.78 (from June 2020): Old drag/sliders functions that
    //took a 'float power > 1.0f' argument instead of
    //ImGuiSliderFlags_Logarithmic. See github.com/ocornut/imgui/issues/3361 for
    //details. IMGUI_API bool      DragScalar(const char* label, ImGuiDataType
    // data_type, void* p_data, float v_speed, const void* p_min, const void*
    // p_max, const char* format, float power = 1.0f) // OBSOLETED in 1.78 (from
    // June 2020) IMGUI_API bool      DragScalarN(const char* label,
    // ImGuiDataType data_type, void* p_data, int components, float v_speed,
    // const void* p_min, const void* p_max, const char* format, float power
    // = 1.0f);                                          // OBSOLETED in 1.78
    // (from June 2020) IMGUI_API bool      SliderScalar(const char* label,
    // ImGuiDataType data_type, void* p_data, const void* p_min, const void*
    // p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78
    // (from June 2020) IMGUI_API bool      SliderScalarN(const char* label,
    // ImGuiDataType data_type, void* p_data, int components, const void* p_min,
    // const void* p_max, const char* format, float power = 1.0f); // OBSOLETED
    // in 1.78 (from June 2020) static inline bool  DragFloat(const char* label,
    // float* v, float v_speed, float v_min, float v_max, const char* format,
    // float power = 1.0f)    { return DragScalar(label, ImGuiDataType_Float, v,
    ...
    // = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed,
    // &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from June 2020)
    // static inline bool  DragFloat3(const char* label, float v[3], float
    // v_speed, float v_min, float v_max, const char* format, float power
    // = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 3, v_speed,
    // &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from June 2020)
    // static inline bool  DragFloat4(const char* label, float v[4], float
    // v_speed, float v_min, float v_max, const char* format, float power
    // = 1.0f) { return DragScalarN(label, ImGuiDataType_Float, v, 4, v_speed,
    // &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from June 2020)
    // static inline bool  SliderFloat(const char* label, float* v, float v_min,
    // float v_max, const char* format, float power = 1.0f)                 {
    // return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max,
    // format, power); }            // OBSOLETED in 1.78 (from June 2020) static
    // inline bool  SliderFloat2(const char* label, float v[2], float v_min,
    // float v_max, const char* format, float power = 1.0f)              {
    // return SliderScalarN(label, ImGuiDataType_Float, v, 2, &v_min, &v_max,
    // format, power); }        // OBSOLETED in 1.78 (from June 2020) static
    // inline bool  SliderFloat3(const char* label, float v[3], float v_min,
    // float v_max, const char* format, float power = 1.0f)              {
    // return SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max,
    // format, power); }        // OBSOLETED in 1.78 (from June 2020) static
    // inline bool  SliderFloat4(const char* label, float v[4], float v_min,
    // float v_max, const char* format, float power = 1.0f)              {
    ...
    // ImGuiPopupFlags_NoOpenOverItems)); } // OBSOLETED in 1.77 (from June
    // 2020) static inline void  TreeAdvanceToLabelPos()               {
    // SetCursorPosX(GetCursorPosX() + GetTreeNodeToLabelSpacing()); }   //
    // OBSOLETED in 1.72 (from July 2019) static inline void
    // SetNextTreeNodeOpen(bool open, ImGuiCond cond = 0) {
    // SetNextItemOpen(open, cond); }                       // OBSOLETED in 1.71
    // (from June 2019) static inline float GetContentRegionAvailWidth() {
    // return GetContentRegionAvail().x; }                               //
    // OBSOLETED in 1.70 (from May 2019) static inline ImDrawList*
    // GetOverlayDrawList()            { return GetForegroundDrawList(); } //
    ...
    // Nov 2018) static inline bool  IsItemDeactivatedAfterChange()        {
    ...
    // in 1.60 (between Dec 2017 and Apr 2018) static inline void
    // ShowTestWindow()                      { return ShowDemoWindow(); } //
    ...
    // SetNextWindowContentWidth(float w)    {
    // SetNextWindowContentSize(ImVec2(w, 0.0f)); }                      //
    // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017) static inline float
    // GetItemsLineHeightWithSpacing()       { return
    // GetFrameHeightWithSpacing(); }                             // OBSOLETED
    // in 1.53 (between Oct 2017 and Dec 2017) IMGUI_API bool      Begin(char*
    // name, bool* p_open, ImVec2 size_first_use, float bg_alpha = -1.0f,
    // ImGuiWindowFlags flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct
    // 2017): Equivalent of using SetNextWindowSize(size,
    // ImGuiCond_FirstUseEver) and SetNextWindowBgAlpha(). static inline bool
    // IsRootWindowOrAnyChildHovered()       { return
    // IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows); }  // OBSOLETED
    // in 1.52 (between Aug 2017 and Oct 2017) static inline void
    ...
    // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017): This was misleading
    // and partly broken. You probably want to use the io.WantCaptureMouse flag
    // instead. static inline bool  IsMouseHoveringAnyWindow()            {
    // return IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            //
    // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017) static inline bool
    // IsMouseHoveringWindow()               { return
    // IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup |
    // ImGuiHoveredFlags_AllowWhenBlockedByActiveItem); }       // OBSOLETED
    // in 1.51 (between Jun 2017 and Aug 2017)
    //-- OBSOLETED in 1.50 and before
    // static inline bool  CollapsingHeader(char* label, const char* str_id,
    // bool framed = true, bool default_open = false) { return
    // CollapsingHeader(label, (default_open ? (1 << 5) : 0)); } // OBSOLETED
    // in 1.49 static inline ImFont*GetWindowFont()                      {
    // return GetFont(); }                                               //
    // OBSOLETED in 1.48 static inline float GetWindowFontSize() { return
    // GetFontSize(); }                                           // OBSOLETED
    // in 1.48 static inline void  SetScrollPosHere()                    {
    // SetScrollHere(); }                                                //
    // OBSOLETED in 1.42
}  // namespace ImGui
